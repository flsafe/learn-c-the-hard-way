...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS){
		...
		if(info.scheme){
			...
			rc = Shell_exec(CURL_SH, "URL", "TARGET", depends_file, NULL);
			check(rc == 0, "Curl failed.");
		} else {
			depends_file = info.path;
		}

		// recusively process the devpkg list
		log_info("Build according to DEPENDS: %s", url);
...
int Command_build(apr_pool_t *p, const char *url, const char *config_opts,
		const char *make_opts, const char *install_opts)
{
	...
	if(access(CONFIG_SCRIPT, X_OK) == 0){
		...
		rc = Shell_exec(CONFIGURE_SH, "OPTS", config_opts, NULL);
		check(rc == 0, "Failed to configure.");
	} else {
		log_warn("There is no configue script %s or it is not executable.", CONFIG_SCRIPT);
	}

	rc = Shell_exec(MAKE_SH, "OPTS", make_opts, NULL);
	check(rc == 0, "Failed to build.");

	log_info("Finished building srouce in %s", BUILD_DIR);
...
int Command_install(apr_pool_t *p, const char *url,
		const char *config_opts, const char *make_opts, const char *install_opts)
{
	int rc = 0;
	check(Shell_exec(CLEANUP_SH, NULL) == 0, "Failed to clean up before building.");

	rc = DB_find(url);
	check(-1 == rc, "Error checking the install database");
	...
	if (1 == rc){
		...
		check(0 == rc, "Failed to build: %s", url)
		...
		sentinel("Installed failed: %s", url);
	}
